...
def generate_tracks():
    tracks = [['Lisboa','Madrid','pink','standard',3],\
              ...
              ['Kharkov','Rostov','green','standard',2],\
              ['Kharkov','Kyiv','grey','standard',4],\
              ['Kharkov','Moskva','grey','standard',4],\
              ]
    ...
    for t in tracks:
        if len(t)==5:
            t.append(0)
    for t in tracks:        
        #print(points_dict[t[4]])
        t.append(points_dict[t[4]])
    for t in tracks:
        nodes = [t[0],t[1]]
        nodes.sort()
        nodes = nodes + [t[2],t[3]]
        nodes = ','.join(nodes)
        if nodes in track_names:
            nodes = nodes + '2'
        track_names.append(nodes)
        t.append(nodes)
...
def get_data():
    ...
    df_tracks = pd.DataFrame(tracks,columns = ['station1','station2','color','type','len','engines','points','track_name'])    
